...
- (void)didExecuteTransitionFrom:(SMState *)from to:(SMState *)to withEvent:(NSString *)event {
    ...
    if( [self.machineWatcher respondsToSelector:@selector(didExecuteTransitionFrom:to:withEvent:)] ){
        [self.machineWatcher didExecuteTransitionFrom:from to:to withEvent:event];
    }
}
